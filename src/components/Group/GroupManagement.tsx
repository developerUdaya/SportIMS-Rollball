...
import { Users, Plus, Save, Shuffle, AlertCircle, Trophy } from 'lucide-react';
import { useData } from '../../context/DataContext';

const GroupManagement: React.FC = () => {
  const { events, teams, groups, addGroup, assignTeamToGroup } = useData();
  ...
  const saveGroupAssignments = async () => {
    ...
    try {
      ...
      // In a real app, you'd have a method to clear existing groups
      ...
      for (const [teamId, groupName] of Object.entries(groupAssignments)) {
        if (groupName) {
          const group = groups.find(g => g.name === `Group ${groupName}` && g.eventId === selectedEventId);
          if (group) {
            assignTeamToGroup(teamId, group.id);
          }
        }
      }
